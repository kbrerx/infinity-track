...
export async function GET(req: Request) {
  try {
    ...
    spendData?.forEach(s => {
      ...
      as.spend += Number(s.spend);
      as.clicks += Number(s.clicks);
    ...
    purchaseData?.forEach(p => {
      ...
      if (c) {
        c.revenue += Number(p.amount);
        c.ventas += 1;
        const as = c.adSets[p.adset_id];
        if (as) {
          as.revenue += Number(p.amount);
          as.ventas += 1;
